...
### 2. 决策树的后剪枝
后剪枝的方法是在生成决策树之后，将“冗余的”的叶节点修剪掉，“冗余”是相对的，因为此时算法使用的数据集从**训练集**变为**测试集**，因此这里的冗余是对于测试集和特定的代价函数而言的，下面介绍代价函数。
...
- b. 从叶节点向上回溯到其父节点，比较将该子树剪枝后的整体的代价函数$C_{alpha}(T_B)$与剪枝之前的代价函数$C_{\alpha}(T_A)$的大小，如果代价函数变小或者相等，则使用剪枝之后的模型；如果变大了，则模型不变，找别的叶节点进行剪枝操作。
